...
echo $(echo -e "${r}4.About${h}")
read -p "5.Exit the installation[1,2,3...]:" key 
case $key in
  1)
    clear
...
#选择安装系统的分区，自动格式化并且挂载
    ...
    if [[ ${tasks} == 2 ]];then
            ...
            echo;
                ...
                pacstrap /mnt linux-firmware linux-headers ntfs-3g networkmanager net-tools dhcpcd vim grub  # 第二部分 分开安装，避免可不必要的错误！
            echo;
	        sleep 3
